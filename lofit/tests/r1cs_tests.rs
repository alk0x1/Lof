...
/// Helper to create R1CS file format in memory
fn create_r1cs_bytes(
    ...
    for (a_terms, b_terms, c_terms) in constraints {
        ...
        for (var_idx, coeff) in a_terms {
            ...
            // Serialize BigInt: length, then signed bytes
...
fn test_r1cs_with_multiple_constraints() {
    ...
    // Check first constraint: (x + y) * 1 = temp
    ...
    // Check second constraint: temp * x = result
...
fn test_r1cs_with_unicode_variable_names() {
    // Test that variable names with unicode work correctly
    let bytes = create_r1cs_bytes(
        &["alpha", "beta"],
        &["gamma"],
        &[(vec![(1, 1)], vec![(2, 1)], vec![(3, 1)])],
...
fn test_truncated_file() {
    ...
    bytes.extend_from_slice(b"lof-r1cs");
    bytes.extend_from_slice(&1u32.to_le_bytes());
    bytes.extend_from_slice(&1u32.to_le_bytes()); // claims 1 public input
                                                  // But file ends here - truncated
...
fn test_r1cs_with_many_constraints() {
    // Create an R1CS with 100 simple constraints
...
fn test_r1cs_only_public_inputs() {
    // R1CS with constraints but no witnesses
    let bytes = create_r1cs_bytes(
        &["a", "b"],
        &[],
        &[
