...
fn test_empty_circuit() {
    ...
    assert!(result.is_ok());
    // Should have at least ONE variable allocated
...
fn test_subtraction_circuit() {
    ...
    // Circuit generation succeeds, but field arithmetic with negative coefficients
    // requires proper handling in the circuit module
    assert!(result.is_ok());
}
...
fn test_unsatisfied_constraint() {
    // Circuit with wrong witness value
...
fn test_unsatisfied_addition() {
    // Circuit: (a + b) * 1 = c, but with wrong c
    let circuit = LofCircuit {
        public_inputs: vec![fr(3), fr(4)],
...
fn test_variable_ordering() {
    // Ensure variables are allocated in correct order: ONE, public inputs, witnesses
    // Note: Witnesses are only allocated if referenced in constraints
    let circuit = LofCircuit {
        public_inputs: vec![fr(1), fr(2), fr(3)],
        witness: vec![fr(4), fr(5)],
        constraints: vec![
            // Add constraints that reference the witness variables
            Constraint {
...
fn test_insufficient_witness_values() {
    // Circuit needs more witness variables than provided
    // The circuit should pad with zeros
    let circuit = LofCircuit {
        public_inputs: vec![fr(1)],
        witness: vec![fr(2)], // Only 1 witness provided
        constraints: vec![Constraint {
    ...
    // Should succeed (padding with zero)
    assert!(result.is_ok());
}

#[test]
fn test_more_witnesses_than_needed() {
    // Provide more witnesses than needed by constraints
    let circuit = LofCircuit {
        public_inputs: vec![fr(2), fr(3)],
        witness: vec![fr(6), fr(999), fr(888)], // Extra witnesses
        constraints: vec![Constraint {
            a: lc(vec![(1, 1)]), // a
            b: lc(vec![(2, 1)]), // b
...
fn test_many_constraints() {
    // Create a circuit with many repeated constraints
    let mut constraints = Vec::new();
    let mut witnesses = Vec::new();

    // Create 50 constraints: a * b = w_i
    for i in 0..50 {
...
fn test_many_public_inputs() {
    // Circuit with many public inputs
...
fn test_simple_let_binding_pattern() {
    ...
    // Matches actual Lof circuit pattern
    let circuit = LofCircuit {
        public_inputs: vec![fr(3), fr(4)],
...
fn test_square_pattern() {
    // Circuit: a * a = a_squared
    let circuit = LofCircuit {
        public_inputs: vec![fr(7)],
        witness: vec![fr(49)],
        constraints: vec![Constraint {
            a: lc(vec![(1, 1)]), // a
            b: lc(vec![(1, 1)]), // a (same variable)
