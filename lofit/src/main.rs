...
fn parse_inputs_in_order(json_map: &InputsJson, variable_names: &[String]) -> Result<Vec<Fr>, Box<dyn std::error::Error>> {
  ...
  match cli.command {
    ...
    Commands::Setup { input, proving_key, verification_key } => {
      ...
      let proving_key_path = proving_key.unwrap_or_else(|| {
        keys_dir.join(format!("{}_pk.bin", base_name))
      });
      
      let verification_key_path = verification_key.unwrap_or_else(|| {
        keys_dir.join(format!("{}_vk.bin", base_name))
      });
      ...
      info!("Writing verification key to {}", verification_key_path.display());
    ...
    Commands::Prove { input, proving_key, public_inputs, witness, output } => {
      ...
      info!("Reading public inputs from {}", public_inputs_path.display());
      ...
      // Save full witness in the proofs directory
      let full_witness_map: HashMap<String, String> = wit_values
        .iter()
        .enumerate()
      ...
      serde_json::to_writer_pretty(
        File::create(&witness_output_path)?,
        &full_witness_map
      )?;
    ...
    Commands::Verify { verification_key, proof, public_inputs, input } => {
      // Auto-detect file paths if not provided
      let base_name = if let Some(input_path) = &input {
        ...
        let key_files: Result<Vec<String>, _> = std::fs::read_dir("keys")
          .map(|entries| {
            entries
              ...
              .filter_map(|entry| {
                ...
                if let Some(filename) = path.file_name().and_then(|s| s.to_str()) {
                  if filename.ends_with("_vk.bin") {
                    // Remove _vk.bin suffix to get base name
                    Some(filename[..filename.len()-7].to_string())
                  } else {
                    None
                  }
                } else {
      ...
      info!("Reading verification key from {}", verification_key_path.display());
      let vk_contents = std::fs::read(&verification_key_path)?;
      debug!("Read {} bytes from verification key file", vk_contents.len());
