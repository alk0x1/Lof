...
// Helper function to convert num_bigint::BigInt to arkworks field element
fn bigint_to_field<F: PrimeField>(big: &BigInt) -> Result<F, SynthesisError> {
    ...
    // Convert via bytes - serialize our u64 limbs to bytes and deserialize as BigInt
    let mut all_bytes = [0u8; 32]; // 4 limbs * 8 bytes = 32 bytes
    ...
    // Convert to field element using PrimeField's from_bigint
