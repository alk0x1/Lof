...
impl<T: Iterator<Item = Token>> Parser<T> {
    ...
    /// Grammar: `TopLevelDeclaration ::= Proof | Component | FunctionDefinition | TypeAlias | EnumDef`
    ...
    fn parse_type(&mut self) -> ParseResult<Type> {
        ...
        match next_token {
            Token::Keyword(Keyword::Field) => {
                self.tokens.next();
                // When parsing a type annotation, default to unconstrained
                // It will be promoted to constrained based on usage
                Ok(Type::Field {
    ...
    /// Grammar: `TypeAlias ::= "type" Identifier "=" Type`
    /// or `EnumDef ::= "type" Identifier "=" "|" EnumVariant ("|" EnumVariant)*`
    fn parse_type_or_enum_definition(&mut self) -> ParseResult<Expression> {
        ...
        // Check if this is an enum (starts with |) or a type alias
        if let Some(Token::Symbol(Symbol::Pipe)) = self.peek() {
            // Parse as enum
    ...
    /// Grammar: `EnumDef ::= "enum" Identifier "{" ("|"? Identifier ("(" Type ("," Type)* ")")?)* "}"`
