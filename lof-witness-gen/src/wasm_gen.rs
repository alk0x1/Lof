/// WASM Witness Calculator Generator
///
/// Generates WASM-enabled witness calculators that can run in the browser

use lof::IRCircuit;
use std::fmt::Write;

pub fn generate_wasm_witness_calculator(circuit: &IRCircuit) -> Result<String, Box<dyn std::error::Error>> {
    let mut code = String::new();

    // Header with WASM imports
    writeln!(&mut code, "//! Generated WASM witness calculator for circuit: {}", circuit.name)?;
    writeln!(&mut code, "//! This code executes the circuit logic in the browser to compute witness values.")?;
    writeln!(&mut code)?;
    ...
    // Input/Output structures for JS interop
    ...
    for (name, _) in &circuit.witnesses {
        writeln!(&mut code, "    #[serde(skip_serializing_if = \"Option::is_none\")]")?;
    ...
    // Main WASM function
    writeln!(&mut code, "/// Compute witness from inputs (WASM entry point)")?;
    writeln!(&mut code, "///")?;
    writeln!(&mut code, "/// This function is called from JavaScript with input values.")?;
    writeln!(&mut code, "/// It returns a complete witness including all intermediate values.")?;
    writeln!(&mut code, "#[wasm_bindgen]")?;
    writeln!(&mut code, "pub fn compute_witness(inputs_js: JsValue) -> Result<JsValue, JsValue> {{")?;
    writeln!(&mut code, "    // Parse inputs from JavaScript")?;
    writeln!(&mut code, "    let inputs: WitnessInputs = serde_wasm_bindgen::from_value(inputs_js)")?;
    writeln!(&mut code, "        .map_err(|e| JsValue::from_str(&format!(\"Failed to parse inputs: {{}}\", e)))?;")?;
    writeln!(&mut code)?;
    ...
    for (name, _) in &circuit.pub_inputs {
        writeln!(&mut code, "    witness.insert(\"{}\".to_string(), inputs.{}.clone());", name, name)?;
    }
    writeln!(&mut code)?;

    // Initialize provided witnesses
    writeln!(&mut code, "    // Provided witness values (if any)")?;
    for (name, _) in &circuit.witnesses {
        writeln!(&mut code, "    if let Some(ref val) = inputs.{} {{", name)?;
        writeln!(&mut code, "        witness.insert(\"{}\".to_string(), val.clone());", name)?;
        writeln!(&mut code, "    }}")?;
    }
    writeln!(&mut code)?;

    // Execute instructions (simplified - we'll generate inline code)
    ...
    for (name, _) in &circuit.pub_inputs {
        writeln!(&mut code, "        {}: witness.get(\"{}\").cloned().unwrap_or_else(|| \"0\".to_string()),", name, name)?;
    }
    for (name, _) in &circuit.witnesses {
        writeln!(&mut code, "        {}: witness.get(\"{}\").cloned().unwrap_or_else(|| \"0\".to_string()),", name, name)?;
    }
...
fn write_wasm_instruction(
    ...
    match instruction {
        ...
        lof::IRInstruction::Assert { condition } => {
            ...
            writeln!(code, "        return Err(JsValue::from_str(\"Assertion failed at instruction {}\"));", index)?;
            writeln!(code, "    }}")?;
        }
...
fn expr_to_js_code(expr: &lof::IRExpr) -> Result<String, Box<dyn std::error::Error>> {
    Ok(match expr {
        ...
        lof::IRExpr::Variable(name) => format!("witness.get(\"{}\").cloned().unwrap_or_else(|| \"0\".to_string())", name),
        lof::IRExpr::Add(l, r) => {
            let left = expr_to_js_code(l)?;
            let right = expr_to_js_code(r)?;
            format!("(({}).parse::<i64>().unwrap() + ({}).parse::<i64>().unwrap()).to_string()", left, right)
        }
        lof::IRExpr::Sub(l, r) => {
            let left = expr_to_js_code(l)?;
            let right = expr_to_js_code(r)?;
        ...
        _ => "String::from(\"0\")".to_string(), // Simplified for now
    })
}
