...
fn run() -> Result<(), Box<dyn std::error::Error>> {
  ...
  match cli.command {
    ...
    Commands::Compile { file, verbose, target, output, generate_templates } => {
        if file.extension().and_then(|ext| ext.to_str()) != Some("lof") {
          let err_msg = "File must have .lof extension";
          error!("{}", err_msg);
          return Err(err_msg.into());
        ...
        match target {
            Target::R1cs => {
                compile_r1cs(&file, verbose, generate_templates)
            }
            Target::Wasm => {
                compile_wasm(&file, verbose, output)
            }
        }
    }
...
fn compile_r1cs(file: &PathBuf, verbose: bool, generate_templates: bool) -> Result<(), Box<dyn std::error::Error>> {
...
fn compile_wasm(file: &PathBuf, verbose: bool, output: Option<PathBuf>) -> Result<(), Box<dyn std::error::Error>> {
    let circuit_name = file.file_stem().unwrap().to_str().unwrap();
    let file_dir = file.parent().unwrap_or_else(|| std::path::Path::new("."));
    
    info!("Compiling to WASM: {}", file.display());
    println!("{} {} {}", "Compiling".blue(), file.display(), "→ WASM".yellow());
    ...
    println!("{} {}", "Generated WASM witness calculator in:".green(), wasm_dir.display());
    ...
    println!("  {}", format!("import init, {{ WitnessCalculator }} from './{}/pkg/{}_witness_calculator.js';", 
             wasm_dir.display(), circuit_name.to_lowercase()).cyan());
    println!("  {}", "await init();".cyan());
    println!("  {}", "const calculator = new WitnessCalculator();".cyan());
    println!("  {}", r#"const witness = calculator.calculate_witness('{"x": "42"}');"#.cyan());
    
    println!("\n{}", "Node.js CLI:".bold());
    println!("  {}", format!("cd {}", wasm_dir.display()).cyan());
    println!("  {}", r#"echo '{"x": "42"}' > input.json"#.cyan());
    println!("  {}", "node generate_witness.js input.json witness.json".cyan());
    
    println!("\n{}", "Browser Demo:".bold());
    println!("  {}", format!("Open {}/example.html in your browser", wasm_dir.display()).cyan());
    
    println!("\n{}", "Integration with lofit:".bold());
    println!("  {}", "const witness = await calculateWitness(inputs);  // WASM (fast)".cyan());
    println!("  {}", "const proof = await lofit.prove(pk, witness, publicInputs);  // JS".cyan());
